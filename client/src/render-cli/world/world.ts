...
import { RAPIER, getWorld } from "@shared/utils/physics";
...
export function setupWorld(
  ...
  const floorRigidBody = getWorld().createRigidBody(floorBodyDesc);
  const floorColliderDesc = RAPIER.ColliderDesc.cuboid(
    floorSize / 2,
    0.1,
    floorSize / 2
  );
  getWorld().createCollider(floorColliderDesc, floorRigidBody);
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const boxRigidBody = getWorld().createRigidBody(boxBodyDesc);
    const boxColliderDesc = RAPIER.ColliderDesc.cuboid(25, 25, 25); // Половина размера геометрии
    getWorld().createCollider(boxColliderDesc, boxRigidBody);
  }
}
